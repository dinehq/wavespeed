...
const FEATURED_MODELS: (CatalogModel & { image: typeof thumb1 })[] =
  ALL_MODELS.slice(0, 16).map((m, i) => ({
    ...m,
    image: [
      thumb1,
      thumb2,
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mt-10 grid gap-8 md:grid-cols-[230px_minmax(0,1fr)]">
            ...
            <section>
              ...
              {(() => {
                const models = isFiltering ? filteredModels : FEATURED_MODELS;
                const featuredImageMap = new Map(
                  FEATURED_MODELS.map((m) => [m.id, m.image]),
                );
                return (
                  <div className="mt-5 grid gap-4 md:grid-cols-2">
                    {models.length === 0 ? (
                      ...
                      models.map((item) => {
                        const image = isFiltering
                          ? null
                          : featuredImageMap.get(item.id);
                        return (
                          <a
                            key={item.id}
                            href="/models/google/nano-banana-pro/edit"
                            className={cn(
                              "bg-surface hover:bg-surface/80 group overflow-hidden transition-colors",
                              image ? "grid grid-cols-[auto_1fr]" : "flex",
                            )}
                          >
                            {image && (
